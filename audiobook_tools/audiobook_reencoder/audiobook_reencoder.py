...
#
def reencode_audio_file(logger, spinner, log_files, audio_file_data, file_count, total_count):
    """
    ...
    #  files_complete = []
    #  files_complete = []
    #  files_compilete.append({'file': 'ss', 'status': 'eeee'})
    ...
    if no_need_to_reencode:
        ...
        if chapter_it:
            ...
            if debug:
                ...
                #  files_complete.append({'file': ffmpeg_input_old, 'status': 'encoding'})
                zzz = ''
...
def print_summary(config): #common_vars):
    ...
    print info
    """
    #  [count, duration] = total_data
    #  [logger, config, spinner, path, log_filenames, debug] = common_vars
    #  print(config['preferred']['threads'])
...
#! fix me
        audio_file_data = process_directory(logger, log_filenames, dirpath, audio_file, audio_file_data, True)
            ...
            # if audio_file_data['cover_art_embeded']:
                # cover_art_file = extract_cover_art_ffmpeg(logger, audio_file, True)           
        #  total_duration = audio_file_data['duration']
...
def add_to_queue(q):
    while True:
        try:
            #  [name, path] = q.get()
            [logger, spinner, log_filenames, audio_file_data, file_count, total_count, dirpath,file_name] = q.get()

            success = reencode_audio_file(logger, spinner, log_filenames, audio_file_data[os.path.join(dirpath,file_name)], file_count, total_count)
            #  import time
...
def encode_files(logger, config, spinner, log_filenames, path, audio_file_data):
    ...
    if os.path.isfile(path):
        ...
        success = reencode_audio_file(logger, spinner, log_filenames, audio_file_data[path], file_count, total_count)
        if not success:
                ...
                if 'read_data_failed' in audio_file_data:
                    ...
                    if os.path.join(dirpath, file_name) in audio_file_data:
                        if not (config['preferred']['only_extract_cover_art'] or config['preferred']['only_delete_unneeded_files']):
                            ...
                            q.put([logger, spinner, log_filenames, audio_file_data, file_count, total_count, dirpath, file_name])
    ...
    #  pprint.pprint(files_complete)
    #  print("----------------")
    #  files_count_past = total_count_to_encode - q.qsize()

    #  files_list_printed_past = []
    #  for i in range(total_count_to_encode):
        #  files_list_printed_past.append(False)

    time.sleep(1)
    #  queue_size_total = q.qsize()
    ...
    while not q.empty():
        ...
        if not files_count_past == files_count_current and len(files_complete) > 0:
            ...
            last_file = files_complete[-1]
            # print newly completed files
            #  if files_count_past == 0:
            #      x = 1
            #  else:
            #  print("---------                                        ")
            #  print("-Encoding: (" + str(files_count_current) + " of " + str(total_count_to_encode) + "; current=" + str(files_count_current) + "; past=" + str(files_count_past) )
            #  print("      current=" + str(files_count_current) + "; past=" + str(files_count_past))


            #  for i in range(files_count_past-1, files_count_current):
                #  print("i=" + str(i ) + "; q=" + str(q.qsize()) + "; current=" + str(files_count_current) + "; past=" + str(files_count_past))
                #  print(" -> " + files_complete[i-1]['file'] )
 
            #  x = files_count_past # len(files_list_past)
            #  print("x=" + str(x) + "                                  ")
            #  print(">>x=" + str(x ) + "; q=" + str(q.qsize()) + "; current=" + str(files_count_current) + "; past=" + str(files_count_past))
            #  while x <= len(files_complete):
            #      print("x=" + str(x ) + "; q=" + str(q.qsize()) + "; current=" + str(files_count_current) + "; past=" + str(files_count_past))
            #      print(" -> " + files_complete[x ]['file'] )
            #      x += 1
            #  #  pprint.pprint(files_complete)
            # restart spinner
            spinner = Spinner()
        #      pprint.pprint(change_list)
            spinner.start("➤ Encoding: (" + str(files_count_current) + " of " + str(total_count_to_encode) + ")" )
            #  files_count_past = files_count_current
        #  files_count_current =  len(files_complete) # total_count_to_encode - q.qsize()
            #  files_list_past = files_complete
        #  finished_total = queue_size_total - q.qsize()
        #  files_complete_sofar = files_complete
        #  time.sleep(5)
...
def main():
    ...
    for i in range(len(files_complete)):
        print("│   " + files_complete[i]["status"] + ": " + files_complete[i]["file"])
        if files_complete[i]['status'] == 'error':
            error_cnt += 1
